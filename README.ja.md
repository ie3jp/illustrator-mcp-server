...
`claude_desktop_config.json` に追加（macOS: `~/Library/Application Support/Claude/claude_desktop_config.json`）:
...
保存後、Claude Desktop を再起動。入力欄にハンマーアイコンが表示されれば OK。
...
E2E テストは新規ドキュメントを作成し、テストオブジェクトを配置して全 45 ケースを自動実行します。
